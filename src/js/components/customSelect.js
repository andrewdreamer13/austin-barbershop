  ...
  selectWrappers.forEach((selectWrapper) => {
    ...
    const state = {
      ...
      selectOption(option) {
        ...
        button.classList.add("is-selected"); // Теперь работает и для мыши, и для клавиатуры!
      ...
      // 2. Новый метод сброса состояния селекта
      reset() {
    ...
    // 3. Автоматически слушаем событие reset у формы
...
function initMouseEvents(selectWrapper, button, options, state) {
  ...
  options.forEach((option) => {
    option.addEventListener("click", (e) => {
      e.stopPropagation();
      state.selectOption(option); // classList.add("is-selected") теперь внутри этого метода
    });
  });
